...
final class BarangTable extends PowerGridComponent
{
    ...
    {
        $actions = [];
        if (auth()->user()->can('update-barang')) {
    ...
    public function header(): array
    {
        ...
        if (auth()->user()->can('export-barang')) {
    ...
    public function bulkExportPdf()
    {
        ...
        $selectedRows = $this->datasource()->whereIn('barang.id', $this->checkboxValues)->get();
        ...
        // Membuat file pdf
        $pdf = Pdf::loadView('pdf.barang', ['datasource' => $selectedRows]);
        // Menyimpan file pdf ke folder pdf
        $pdf->save($path . '/barang.pdf');
        // Menampilkan file pdf
        // return response()->download($path . '/barang.pdf');
