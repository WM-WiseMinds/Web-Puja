...
final class JenisSampahTable extends PowerGridComponent
{
    use WithExport;
    ...
    public function filters(): array
    {
        return [
            Filter::select('status', 'status')
                ->dataSource(JenisSampah::all()->unique('status'))
                ->optionValue('status')
                ->optionLabel('status'),
        ];
    }

    public function actions(JenisSampah $row): array
    {
        $actions = [];

        if (auth()->user()->can('update-jenis-sampah')) {
            $actions[] = Button::add('edit')
                ...
                ->openModal('jenis-sampah-form', ['rowId' => $row->id]);
        }
    ...
    public function header(): array
    {
        ...
        if (auth()->user()->can('create-jenis-sampah')) {
            $header[] = Button::add('add-jenis-sampah')
                ...
                ->openModal('jenis-sampah-form', []);
        }

        if (auth()->user()->can('export-jenis-sampah')) {
    ...
    public function exportPdf()
    {
        ...
        // Membuat file pdf
        $pdf = Pdf::loadView('pdf.jenis-sampah', ['datasource' => $datasource]);
        // Menyimpan file pdf ke folder pdf
        $pdf->save($path . '/jenis-sampah.pdf');
        // Menampilkan file pdf
        return response()->download($path . '/jenis-sampah.pdf');
    }

    // Function to delete data
    public function delete($rowId)
    {
        $jenisSampah = JenisSampah::findOrFail($rowId);
        $jenisSampah->delete();
        $this->success('Jenis Sampah berhasil dihapus');
    }
}
